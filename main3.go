...
type PersonCat struct {
    Name        string      `json:"name"`
    FavoriteCat *NullString `json:"favorite_cat"` // Some people don't like dogs ¯\_(ツ)_/¯
}
    ...
    schema, err := graphql.NewSchema(graphql.SchemaConfig{
        Query: graphql.NewObject(graphql.ObjectConfig{
            ...
            Fields: graphql.Fields{
                ...
                "catpeople": &graphql.Field{
                    Type: graphql.NewList(CatType),
                    Args: graphql.FieldConfigArgument{
                        "favorite_cat": &graphql.ArgumentConfig{
                            Type: NullableString,
                        },
                    },
                    Resolve: func(p graphql.ResolveParams) (interface{}, error) {
                        fmt.Println("Cats")
                        cat, catOK := p.Args["favorite_cat"].(*NullString)
                        foo.FavoriteCat = cat
                        people := []PersonCat{
                            PersonCat{Name: "Alice", FavoriteCat: NewNullString("Yorkshire Terrier")},
                            // `Bob`'s favorite dog will be saved as null in the database
                            PersonCat{Name: "Bob", FavoriteCat: NewNullString("")},
                            PersonCat{Name: "Chris", FavoriteCat: NewNullString("French Bulldog")},
                        }
                        switch {
                        case catOK:
                            log.Printf("favorite_dog from arguments: %+v", cat)
                            dogPeople := make([]PersonCat, 0)
                            for _, p := range people {
                                if p.FavoriteCat.Valid {
                                    if p.FavoriteCat.String == cat.String {
                                        dogPeople = append(dogPeople, p)
                                    }
                                }
                            }
...
query {
  people {
    ...
    queryWithArgument := `
query {
  people(favorite_do: "Yorkshire Terrier") {
    name
    favorite_dog
  }
   people(favorite_cat: "ping") {
    name 
    favorite_cat
    }
}`
